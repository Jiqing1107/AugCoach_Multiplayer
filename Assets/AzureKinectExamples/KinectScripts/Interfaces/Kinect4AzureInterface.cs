...
namespace com.rfilkov.kinect
{
    ...
    public class Kinect4AzureInterface : DepthSensorBase
    {
        ...
        public override BaseSensorSettings GetSensorSettings(BaseSensorSettings settings)
        {
            ...
            //The default mode is play recording
            extSettings.deviceStreamingMode = (int)KinectInterop.DeviceStreamingMode.PlayRecording;
        ...
        public override KinectInterop.SensorData OpenSensor(KinectManager kinectManager, KinectInterop.FrameSource dwFlags, bool bSyncDepthAndColor, bool bSyncBodyAndDepth)
        {
            ...
            if (deviceStreamingMode == KinectInterop.DeviceStreamingMode.PlayRecording)
            {
                ...
                {
                    Debug.Log("Storage access permisson is enabled.");
                }
                else
                {
                    // Request permission for reading external files
                    Permission.RequestUserPermission(Permission.ExternalStorageRead);
                }

                string fileName = "cam0.mkv";
                
                recordingFile = "/storage/emulated/0/" + fileName;
                */
                // You can now use this path to access the file
                // For example, to read the file:
                if (System.IO.File.Exists(recordingFile))
                {
                    Debug.Log("File has been found: " + recordingFile);
                }
                else
                {
                    Debug.LogError("File not found: " + recordingFile);
                }
                Debug.Log("Loading mkv file finished.");
        ...
        public void Pause(bool isPaused) //LG
        {
            if (isPaused)
            {
                ...
                Debug.Log("Pause response time:" + pauseTimeElapsed);
            }
            else
            {
